...
import {
  PaymentCondition,
  PaymentConditionForm as PaymentConditionFormType,
...
} from "@/lib/actions/condicoes-pagamento";
...
function InstallmentTotal({ control }: { control: any }) {
  ...
  const totalPercent = (parcelas || []).reduce(
    (sum, p) => sum + (Number(p.percentual_valor) || 0),
    0
  );
  ...
  return (
    ...
    >
      ...
      <span>
        Soma das parcelas: <strong>{totalPercent.toFixed(2)}%</strong>
      </span>
    </div>
  );
}
...
export default function PaymentConditionForm({
  ...
  const onSubmit = async (data: PaymentConditionFormType) => {
    const numberedData = {
      ...data,
      parcelas: data.parcelas.map((p, index) => ({
        ...p,
        ...
        >
          ...
          <div className="space-y-4">
            {fields.map((field, index) => (
              <div
                key={field.id}
                className="grid grid-cols-1 md:grid-cols-10 gap-2 items-start border p-4 rounded-lg relative"
              >
                <div className="md:col-span-1 font-medium pt-8">
                  #{index + 1}
                </div>
                <FormField
                  control={form.control}
                  name={`parcelas.${index}.dias_vencimento`}
                  render={({ field }) => (
                ...
                <FormField
                  control={form.control}
                  name={`parcelas.${index}.percentual_valor`}
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
          ...
          <Button
            ...
            onClick={() =>
              append({
                numero_parcela: fields.length + 1,
                dias_vencimento: 0,
                percentual_valor: 0,
