...
import { deleteTransportadora } from "@/lib/actions/transportadoras";
import { toast } from "sonner";

export function DeleteTransportadoraButton({ id, children }: { id: number, children: React.ReactNode }) {
  const [isPending, startTransition] = useTransition();

  const handleClick = () => {
    startTransition(async () => {
      const result = await deleteTransportadora(id);
      if (result?.success) {
