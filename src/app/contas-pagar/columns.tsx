...
export const columns: ColumnDef<PurchaseInstallment>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      const vencido = !row.getValue("data_pagamento") && isVencido(data as string);
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const isCancelada = row.original.purchase_ativo === 0 || row.original.purchase_ativo === false;
      return (
        <Button
          variant="outline"
          size="sm"
          disabled={isPago || isCancelada}
        >
          Dar Baixa
        </Button>
      );
