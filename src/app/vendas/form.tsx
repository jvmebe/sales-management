...
interface SaleFormProps {
  ...
  readOnly?: boolean;
  // Props adicionais para o ProductSelectionDialog (Criação Rápida)
  brands: ProductBrand[];
...
export default function SaleForm({
  ...
  initialData,
  readOnly = false,
  // Inicializando com array vazio para evitar crash
  brands = [],
  categories = [],
  units = [],
  suppliers = [],
  ...
  const [selectedNames, setSelectedNames] = useState({
    client: initialData
      ? clients.find((c) => c.id === initialData.client_id)?.nome
      : "",
    employee: initialData
      ? employees.find((e) => e.id === initialData.employee_id)?.nome
      : "",
    condition: initialData
      ? paymentConditions.find((p) => p.id === initialData.payment_condition_id)
          ?.descricao
      : "",
  });
  ...
  const watchedInstallments =
    useWatch({ control: form.control, name: "installments" }) || [];

  const totalVenda = watchedItems.reduce(
    ...
    <Form {...form}>
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <Input
                value={
                  initialData?.numero_nota
                    ? String(initialData.numero_nota).padStart(6, "0")
                    : "Gerado ao Salvar"
                }
                disabled
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <Dialog
                    open={dialogsOpen.client}
                    onOpenChange={(o) =>
                      setDialogsOpen({ ...dialogsOpen, client: o })
                    }
                  >
                    <DialogTrigger asChild>
                      <Button
                        variant="outline"
                        className="justify-start font-normal text-left truncate"
                        disabled={isLocked}
                      >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <Dialog
                    open={dialogsOpen.employee}
                    onOpenChange={(o) =>
                      setDialogsOpen({ ...dialogsOpen, employee: o })
                    }
                  >
                    <DialogTrigger asChild>
                      <Button
            ...
            <div className="grid grid-cols-12 gap-2 items-end border p-3 rounded-lg bg-muted/30">
              <div className="col-span-2 md:col-span-1">
                ...
                <Input
                  ...
                  onClick={() =>
                    setDialogsOpen((p) => ({ ...p, product: true }))
                  }
                  placeholder="ID"
                />
              </div>
              <div className="col-span-8 md:col-span-3">
                ...
                <Dialog
                  ...
                  onOpenChange={(o) =>
                    setDialogsOpen({ ...dialogsOpen, product: o })
                  }
                >
                  <DialogContent className="max-w-6xl w-full h-[90vh]">
                    <ProductSelectionDialog
                      ...
                      onSelect={(p) => handleProductSelected(p as Product)}
                      // Passando as novas props para o formulário de criação
                      brands={brands}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col w-1/3">
                  ...
                  <Dialog
                    open={dialogsOpen.condition}
                    onOpenChange={(o) =>
                      setDialogsOpen({ ...dialogsOpen, condition: o })
                    }
                  >
                    <DialogTrigger asChild>
                      <Button
                ...
                <TableBody>
                  {installments.map((inst, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        {format(
                          new Date(inst.data_vencimento),
                          "dd/MM/yyyy"
                        )}
                      </TableCell>
                      <TableCell>
                        R$ {Number(inst.valor_parcela).toFixed(2)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <FormFooter
          ...
          deleteButton={
            readOnly && initialData?.ativo ? (
              <CancelSaleButton id={initialData.id!} />
            ) : undefined
          }
        />
        {readOnly && (
