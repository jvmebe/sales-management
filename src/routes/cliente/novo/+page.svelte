<script lang="ts">
  ...
  import DatePicker from "$lib/components/date-picker.svelte";
	...
	// Assumindo que você terá estes modais de lista
	...
	// --- Lógica para os Modais de Seleção ---
	let selectedCity = $state({ id: null, nome: '' });
	let selectedPaymentCondition = $state({ id: null, descricao: '' });

	$effect(() => { $formData.cidade_id = selectedCity?.id; });
	$effect(() => { $formData.cond_pag_id = selectedPaymentCondition?.id; });
</script>

<div class="mb-4 flex items-center gap-4">
	<Button href="/cliente" variant="outline" size="icon"><ArrowLeft class="h-4 w-4" /></Button>
	<h1 class="text-2xl font-bold">Novo Cliente</h1>
</div>

<form method="POST" use:enhance class="space-y-6">
	<div class="flex items-center space-x-6 pt-2">
		<Form.Field {form} name="is_juridica" class="flex flex-row items-center space-x-3 space-y-0">
			<Form.Control>
				{#snippet children({ props })}
					<div class="flex items-center gap-2">
						<Checkbox {...props} bind:checked={$formData.is_juridica} />
						<Label>Pessoa Jurídica</Label>
					</div>
				{/snippet}
			</Form.Control>
		</Form.Field>
		<Form.Field {form} name="is_ativo" class="flex flex-row items-center space-x-3 space-y-0">
			<Form.Control>
				{#snippet children({ props })}
					<div class="flex items-center gap-2">
						<Checkbox {...props} bind:checked={$formData.is_ativo} />
						<Label>Ativo</Label>
					</div>
				{/snippet}
			</Form.Control>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
		<FormInput {form} name="nome" label={labelNome} bind:userInput={$formData.nome} />
		<FormInput {form} name="apelido" label={labelApelido} bind:userInput={$formData.apelido} />
	</div>

	<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
		<FormInput {form} name="cpf" label={labelDoc1} bind:userInput={$formData.cpf} />
		<FormInput {form} name="rg" label={labelDoc2} bind:userInput={$formData.rg} />
	...
	<div class="grid grid-cols-1 md:grid-cols-6 gap-4">
		<div class="md:col-span-4">
			<FormInput {form} name="endereco" label="Endereço" bind:userInput={$formData.endereco} />
		</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
		<Form.Field {form} name="cidade_id">
			...
			<div class="flex items-center gap-2 mt-1">
				<Input readonly value={selectedCity?.nome || ''} placeholder="Selecione uma cidade" />
		...
		<Form.Field {form} name="cond_pag_id">
			...
			<div class="flex items-center gap-2 mt-1">
				<Input readonly value={selectedPaymentCondition?.descricao || ''} placeholder="Selecione uma condição" />
				<PaymentConditionListDialog bind:pickedItem={selectedPaymentCondition} {data} />
			</div>
			<Form.FieldErrors />
		</Form.Field>
	</div>
	...
	<div class="max-w-xs">
		<FormInput {form} name="limite_credito" label="Limite de Crédito (R$)" type="number" bind:userInput={$formData.limite_credito} />
	</div>

	<div class="flex justify-end pt-4">
		<Form.Button>Salvar Cliente</Form.Button>
	</div>
</form>