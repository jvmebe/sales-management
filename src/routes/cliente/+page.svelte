<script lang="ts">
	...
	const filteredData = $derived(
		data.clients?.filter((client) => {
			...
			// Busca pelo nome do cliente, email ou cidade
			const nome = (client.nome || '').toLowerCase();
			const email = (client.email || '').toLowerCase();
			const cidade = (client.cidade_nome || '').toLowerCase();

			return nome.includes(search) || email.includes(search) || cidade.includes(search);
		}) ?? []
	);
...
<div class="flex items-center py-4">
	<Input
		placeholder="Filtrar por nome, email ou cidade..."
		value={searchValue}
	...
	<Table.Root>
		...
		<Table.Body>
			{#if pagedData.length > 0}
				{#each pagedData as client (client.id)}
					<Table.Row>
						<Table.Cell class="font-medium">{client.id}</Table.Cell>
						<Table.Cell>{client.nome}</Table.Cell>
						<Table.Cell>{client.email}</Table.Cell>
						<Table.Cell>{client.cidade_nome || 'Não informada'}</Table.Cell>
						<Table.Cell class="text-right">
							<Button variant="outline" size="icon" href={`/cliente/${client.id}`}>
								<Edit class="h-4 w-4" />
