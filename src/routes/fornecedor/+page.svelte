<script lang="ts">
	...
	const filteredData = $derived(
		data.suppliers?.filter((supplier) => {
			...
			const apelido = (supplier.apelido || '').toLowerCase();
			const email = (supplier.email || '').toLowerCase();
			const cidade = (supplier.cidade_nome || '').toLowerCase();
	...
	<Table.Root>
		...
		<Table.Body>
			{#if pagedData.length > 0}
				{#each pagedData as supplier (supplier.id)}
					<Table.Row>
						<Table.Cell class="font-medium">{supplier.id}</Table.Cell>
						<Table.Cell>{supplier.nome}</Table.Cell>
						<Table.Cell>{supplier.apelido}</Table.Cell>
						<Table.Cell>{supplier.email || '-'}</Table.Cell>
						<Table.Cell>{supplier.cidade_nome || 'Não informada'}</Table.Cell>
						<Table.Cell class="text-right">
							<Button variant="outline" size="icon" href={`/fornecedor/${supplier.id}`}>
								<Edit class="h-4 w-4" />
