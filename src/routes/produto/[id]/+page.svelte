<script lang="ts">
	...
	// Lógica para exibir os nomes dos itens selecionados
...
<div class="mb-4 flex items-center gap-4">
	...
	<h1 class="text-2xl font-bold">Editar Produto: {data.product.nome}</h1>
</div>
...
<form method="POST" action="?/update" use:enhance class="space-y-6" id="update-form">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
		<FormInput {form} name="valor_compra" label="Valor de Compra" type="number" bind:userInput={$formData.valor_compra}/>
		<FormInput {form} name="valor_venda" label="Valor de Venda" type="number" bind:userInput={$formData.valor_venda}/>
		<FormInput {form} name="percentual_lucro" label="% Lucro" type="number" bind:userInput={$formData.percentual_lucro}/>
	</div>

	<div class="flex items-end gap-4">
		<FormInput {form} name="estoque" label="Estoque" type="number" bind:userInput={$formData.estoque}/>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
		<div>
			...
			<p class="text-sm mt-1">{formatDate(data.product.data_cadastro)}</p>
		</div>
		<div>
			<Label class="text-sm font-medium text-muted-foreground">Última Edição</Label>
			<p class="text-sm mt-1">{formatDate(data.product.data_ultima_edicao)}</p>
		</div>
	</div>
</form>
