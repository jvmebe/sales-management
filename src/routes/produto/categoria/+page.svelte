<script lang="ts">
	...
	const filteredData = $derived(
		data.categories?.filter((category) => {
			...
			const nome = (category.nome || '').toLowerCase();
			const descricao = (category.descricao || '').toLowerCase();
...
<div class="mb-4 flex justify-between items-center">
	<h1 class="text-2xl font-bold">Categorias de Produto</h1>
	<Button href="/produto/categoria/novo">
	...
	<Table.Root>
		...
		<Table.Body>
			{#if pagedData.length > 0}
				{#each pagedData as category (category.id)}
					<Table.Row>
						<Table.Cell class="font-medium">{category.id}</Table.Cell>
						<Table.Cell>{category.nome}</Table.Cell>
						<Table.Cell>{category.descricao || '-'}</Table.Cell>
                        <Table.Cell>
                            {#if category.ativo}
                                <Badge>Sim</Badge>
                            {:else}
						...
						<Table.Cell class="text-right">
							<Button variant="outline" size="icon" href={`/produto/categoria/${category.id}`}>
								<Edit class="h-4 w-4" />
			...
			{:else}
				<Table.Row>
					<Table.Cell colspan={5} class="h-24 text-center"> Nenhuma categoria encontrada. </Table.Cell>
				</Table.Row>
			{/if}
		</Table.Body>
	</Table.Root>
