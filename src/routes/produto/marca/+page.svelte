<script lang="ts">
	...
	const filteredData = $derived(
		data.brands?.filter((brand) => {
			...
			const nome = (brand.nome || '').toLowerCase();
...
<div class="mb-4 flex justify-between items-center">
	<h1 class="text-2xl font-bold">Marcas de Produto</h1>
	<Button href="/produto/marca/novo">
	...
	<Table.Root>
		...
		<Table.Body>
			{#if pagedData.length > 0}
				{#each pagedData as brand (brand.id)}
					<Table.Row>
						<Table.Cell class="font-medium">{brand.id}</Table.Cell>
						<Table.Cell>{brand.nome}</Table.Cell>
						<Table.Cell>
							{#if brand.ativo}
								<Badge>Sim</Badge>
							{:else}
						...
						<Table.Cell class="text-right">
							<Button variant="outline" size="icon" href={`/produto/marca/${brand.id}`}>
								<Edit class="h-4 w-4" />
			...
			{:else}
				<Table.Row>
					<Table.Cell colspan={4} class="h-24 text-center"> Nenhuma marca encontrada. </Table.Cell>
				</Table.Row>
			{/if}
		</Table.Body>
	</Table.Root>
